 ...
 *
 * Uses { page } fixture (same as 50-user public tests) so
 * video recording is handled automatically by the config.
 *
...
for (let i = 0; i < TEST_ACCOUNTS.length; i++) {
  ...
  base(`Account ${i + 1}: Profile & Card (${account.clientId})`, async ({ page }) => {
    // ── 1. Homepage + age gate ──
